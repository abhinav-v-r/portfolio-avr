export const siteConfig = {
  name: "Abhinav V R",
  title: "Computer Science Engineering Sophomore",
  description: "Portfolio website of Abhinav V R",
  accentColor: "#1d4ed8",
  social: {
    email: "abhinavvr2675@gmail.com",
    linkedin: "https://linkedin.com/in/abhinavvr",
    twitter: "https://x.com/abhinavVR",
    instagram: "https://instagram.com/abhinav_vr_",
    github: "https://github.com/abhinav-v-r",
    resume: "/resume.pdf",
  },
  aboutMe:
    "A Computer Science Engineerng student and AI/ML Developer with a strong interest in building reliable,data-driven solutions. Alongside my technical interests, I am actively involved in leading student communities and clubs, while also contributing to social service and volunteering initiatives,combining technical growth with leadership and social responsibility.",
  skills: ["Python", "TensorFlow", "Keras", "C", "Java","Bash","MySQL", "Designing", "Web Development", "AI/ML", "Problem Solving", "Object-Oriented Programming", "Leadership", "Team Management"],
  projects: [
    {
      name: "Dermal Scan - AI Skin Aging Detection",
      description:
        "An AI-powered application for analyzing facial skin aging signs using EfficientNetB0 deep learning model.",
      link: "https://github.com/abhinav-v-r/Dermal-Scan",
      skills: ["AI/ML", "Deep Learning", "Python", "TensorFlow"],
    },
    {
      name: "Kaaval AI - Cyber Fraud Prevention and Digital Safety Application",
      description:
        "An all-in-one AI-powered digital safety ecosystem designed to protect, educate, and empower users in the digital age, combiningreal-time threat detection with interactive learning to build long-term digital resilience.",
      link: "https://github.com/abhinav-v-r/Kaaval-AI-Intellipat",
      skills: ["React", "Node.js", "Next.js", "Vite", "Manifest V3"],
    },
    {
      name: "Netflix Clone",
      description:
        "A fully responsive web application developed using HTML, CSS, and JavaScript, designed to replicate the core functionalities and visual appeal of Netflix.",
      link: "https://github.com/abhinav-v-r/Netflix-Clone",
      skills: ["HTML", "CSS", "JavaScript"],
    },
    {
      name: "Blood and Organ Donation Management System",
      description:
        "A comprehensive Java desktop application for managing blood and organ donations using Java Swing and MySQL.",
      link: "https://github.com/abhinav-v-r/Blood-and-Organ-Donation-Management",
      skills: ["Java", "MySQL", "Maven", "Swing"],
    },
    {
      name: "Moon Phase Tracker Web App",
      description:
        "A simple and interactive web application that tracks the current moon phase and displays the next 7 days of moon phases with images and labels.",
      link: "https://github.com/abhinav-v-r/moon-phase-tracker",
      skills: ["HTML", "CSS", "JavaScript", "SunCalc"],
    },
  ],
  experience: [
    {
      company: "Infosys Sprigboard",
      title: "Intern",
      dateRange: "Aug 2025 - Oct 2025",
      bullets: [
        "Developed an AI-powered application for analyzing facial skin aging signs",
        "Implemented EfficientNetB0 deep learning model for skin aging detection",
        "Presented the final project to Infosys employees and evaluators",
      ],
    },
    {
      company: "μLearn Foundation",
      title: "District Lead",
      dateRange: "Feb 2026 - Present",
      bullets: [
        "Spearheaded the regional expansion of the μLearn footprint by identifying and establishing new campus chapters across the district",
        "Coordinated Mutate - μLearn Campus Lead Meet, facilitating strategic planning sessions and leadership training for campus leads to synchronize regional goals and community standards.",
      ],
    },
    {
      company: "TinkerHub",
      title: "Learning Coordinator",
      dateRange: "Jul 2025 - Present",
      bullets: [
        "Conducted study jams,learning sessions and hackathons on campus",
        "Encouraged and guided students to actively participate in learning programs",
      ],
    },
    {
      company: "μLearn NSS",
      title: "Campus Lead",
      dateRange: "Aug 2025 - Present",
      bullets: [
        "Initiated and established the μLearn chapter in NSS College of Engineering",
        "Leading campus-level learning sessions,competitions and technical events",
        "Acting as a liaison between the μLearn Foundation and the college",
      ],
    },
    {
      company: "Student Association of Computer Science",
      title: "Codify Head",
      dateRange: "Aug 2025 - Present",
      bullets: [
        "Organized Hackathons,coding competitions and technical events",
        "Led student participation and coordination for club activities",
      ],
    },
  ],
  education: [
    {
      school: "Jawahar Navodaya Vidyalaya Palakkad",
      degree: "Higher Secondary",
      dateRange: "2017 - 2024",
      achievements: [
        "Secured 93% in Class XII Board Examination",
        "Represented School in Regional level Exhibitions",
        "NCC Cadet for 2 years",
      ],
    },
    {
      school: "NSS College of Engineering Palakkad",
      degree: "B.Tech Computer Science and Engineering",
      dateRange: "2024-Present",
      achievements: [
        "CGPA: 9.98",
        "Leading student communities and college clubs",
        "Hackathon Winner | Student Expo Winner | Idea Pitching Winner",
      ],
    },
  ],
};
